...
SlideDeck.prototype.loadConfig_ = function(config) {
  ...
  if (this.config_.presenters) {
    ...
    if (presenters.length == 1) {
      ...
      var email = p.email ? '<span>email</span>' + p.email + ' ' : '';

      var www = p.www ? '<span>web</span><a href="' + p.www +
                        '">' + p.www.replace(/https?:\/\//, '') + '</a>' : '';
	  var slideUrl = p.slideUrl ? '<span>slides</span><a href="' + p.slideUrl +
                 '">' + p.slideUrl.replace(/https?:\/\//, '') + '</a>' : '';

      var github = p.github ? '<span>github</span><a href="' + p.github +
          '">' + p.github.replace(/https?:\/\//, '') + '</a>' : '';

      var html2 = [gplus, twitter, www, email, slideUrl, github].join('<br>');
